...
# 确保 src 目录在 Python 路径中，以便使用绝对导入
...
async def lifespan(_app: FastAPI) -> AsyncGenerator[None]:
    ...
    startup_logger.info("workTranslationAgent 启动完成")

    yield

    startup_logger.info("正在关闭 workTranslationAgent...")

    # await redis_service().close()
    startup_logger.info("Redis 连接已关闭")

    await close_db_engines()
    startup_logger.info("workTranslationAgent 已关闭")


async def health_check() -> dict[str, Any]:
    """健康检查端点"""
    return {"status": "ok", "service": "workTranslationAgent"}


def create_app() -> FastAPI:
    """创建 FastAPI 应用"""
    application = FastAPI(
        title="workTranslationAgent",
