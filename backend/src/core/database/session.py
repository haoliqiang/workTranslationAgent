...
async def run_migrations() -> None:
    ...
    if not alembic_ini.exists():
        logger.warning("Alembic 未配置 (alembic.ini 不存在)，跳过数据库迁移")
        logger.warning("如需使用数据库迁移，请运行: alembic init alembic")
        return
    ...
    try:
        ...
        error_output = exc.stderr or exc.stdout or "No error output available"
        logger.error("Migration command failed with exit code %d", exc.returncode)
        logger.error("Migration stderr: %s", error_output)
        if exc.stdout:
            logger.error("Migration stdout: %s", exc.stdout)
        msg = f"Database migration failed: {error_output}"
